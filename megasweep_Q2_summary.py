...
def main():
    ...
    filename = filepath + 'combined_Q2.txt'
    ...
    mega_plot(mode1)
    prettify_figure('Frequency', 'Q2', 'mode 1, steady-state')
    
    mode2 = mega2matrix(raw_data[:,[0,2,-1]])
    mega_plot(mode2)
    prettify_figure('Frequency', 'Q2', 'mode 2, steady-state')
    ...
    prettify_figure('side-band frequency', 'Q2')
    plt.show()
