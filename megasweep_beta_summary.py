...
def main():
    ...
    filename = filepath + 'test_mega_steady_state_beta_carbon.txt'
    ...
    mega_plot(mode1)
    prettify_figure('Frequency', 'beta', 'mode 1, steady-state')
    
    mode2 = mega2matrix(raw_data[:,[0,2,-1]])
    mega_plot(mode2)
    prettify_figure('Frequency', 'beta', 'mode 2, steady-state')
    ...
    prettify_figure('side-band frequency', 'beta')
    plt.show()
