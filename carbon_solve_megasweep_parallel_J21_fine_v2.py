...
def process_input(para):
    ...
    # ==============================================================
    J21 = para
    temp_folder = 'PythonParallelTemp_J21_fine_v2'
    ...
    freq1, fft_out1 = myFFT(time[-int(0.1*len(time)):], Assol[-int(0.1*len(time)):,0]); 
    # only last 10% of the time-domain data, which assuming to have reached steady-state
    
    freq2, fft_out2 = myFFT(time[-int(0.1*len(time)):], Assol[-int(0.1*len(time)):,2]); 
    # only last 10% of the time-domain data, which assuming to have reached steady-state
...
def main():
    para_list = np.logspace(-4.5, -3.5, 200);   
    ...
    with open('combined_J21_fine_v2.txt', 'a') as outfile:
        for para in para_list:
            with open(os.path.join(os.getcwd(),'PythonParallelTemp_J21_fine_v2') + '/' +str(para)+'.txt', 'r') as infile:
                outfile.write(infile.read())
    #st.rmtree(os.path.join(os.getcwd(),'PythonParallelTemp_J21_fine'))
