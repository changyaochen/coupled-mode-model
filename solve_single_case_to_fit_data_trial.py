...
def main():
    ...
    plt.xlim([1.0, 1.1]) 
    ccylib.prettify_figure('FFT amplitude (a.u.)', 'Frequency (Hz)', 'Steady-states', grid = 'on')
    plt.yscale('log') 
    plt.show() 
    ...
    if save_file == 1:
        np.savetxt('solve_single_case_to_fit_data_ss_trial.dat',np.hstack([time[:,np.newaxis], Assol])) 
    if save_file_FFT == 1:
        ...
        with open('solve_single_case_to_fit_data_FFT.txt','a') as FFT_file:
    ...
    if save_file == 1:
        np.savetxt('solve_single_case_to_fit_data_rd_trial.dat',np.hstack([time[:,np.newaxis], Ringdown])) 
