...
def main():
    ...
    if save_file == 1:
        np.savetxt('solve_single_case_to_fit_data_ss.dat',np.hstack([time[:,np.newaxis], Assol])) 
    if save_file_FFT == 1:
    ...
    if save_file == 1:
        np.savetxt('solve_single_case_to_fit_data_rd.dat',np.hstack([time[:,np.newaxis], Ringdown])) 
