...
def process_input(para):
    ...
    if save_file_FFT == 1:
        if not os.path.exists('PythonParallelTemp_J12_fine'):
            os.makedirs('PythonParallelTemp_J12_fine')
        ...
        with open(os.path.join(os.getcwd(),'PythonParallelTemp_J12_fine') + '/' + str(para) + '.txt','w') as FFT_file:
...
def main():
    ...
    with open('combined_J12_fine.txt', 'a') as outfile:
        for para in para_list:
            with open(os.path.join(os.getcwd(),'PythonParallelTemp_J12_fine') + '/' +str(para)+'.txt', 'r') as infile:
