...
def process_input(para):
    ...
    time = np.linspace(0, 10*reference, 200*reference+1)
    ...
    # ==============================================================
    bound = para
    temp_folder = 'PythonParallelTemp_bound_v5'
    if not os.path.exists(temp_folder):
            os.makedirs(temp_folder)
    temp_file = os.path.join(os.getcwd(),temp_folder) + '/ss_bound_' + str(para) + '.png' 
    # ==============================================================
    # ============================================================== 
    ...
    #plt.show()
    plt.savefig(os.path.join(os.getcwd(),temp_folder) + '/ss_FFT_bound_' + str(para) + '.png')
    ...
    temp_file_2 = os.path.join(os.getcwd(),temp_folder) + '/rd_bound_' + str(para) + '.png' 
    ...
    plt.ylim([1.0, 1.1])
    plt.yscale('linear')
    plt.savefig(os.path.join(os.getcwd(),temp_folder) + '/RTSA_mode1_bound_' + str(para) + '.png', dpi = 400)
    
    RTSA(time, Ringdown[:,2], 200)
    prettify_figure('Frequency', 'time', 'real-time spectrum, ringdown, mode 2')
    plt.ylim([1.0, 1.1])  
    plt.yscale('linear')  
    plt.savefig(os.path.join(os.getcwd(),temp_folder) + '/RTSA_mode2_bound_' + str(para) + '.png', dpi = 400)

def main():
    para_list = np.logspace(-5, -3, 50);   
    ...
    #with open('combined_bound_v5.txt', 'a') as outfile:
    #    for para in para_list:
    #        with open(os.path.join(os.getcwd(),'PythonParallelTemp_bound_v5') + '/' +str(para)+'.txt', 'r') as infile:
    #            outfile.write(infile.read())
    #st.rmtree(os.path.join(os.getcwd(),'PythonParallelTemp_bound'))
    for para in para_list:
        process_input(para)
