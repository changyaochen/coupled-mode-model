...
#    print('='*50)
    ...
    temp_folder = 'PythonParallelTemp_NMM_1_to_3_Js_v2'
    ...
    if doPlot:
      ...
      temp_file_q = os.path.join(os.getcwd(),temp_folder) + '/ss_q_Js_' + str(para) + '.png'
      plt.figure(2)
      plt.subplot(211)
      plt.plot(time[::100], Assol[::100,0] + Assol[::100,2], 'b-')
      plt.xlabel('time'); plt.ylabel('displacement'); plt.title('mode 1')
      plt.subplot(212); 
      plt.plot(time[::100], Assol[::100,0] - Assol[::100,2], 'g-')
      plt.xlabel('time'); plt.ylabel('displacement'); plt.title('mode 2')
      #plt.show()
      plt.savefig(temp_file_q)
      plt.close(2)
    ...
    if doPlot:
      ...
      plt.figure(4)
      plt.subplot(211)
      plt.plot(time[::100], Ringdown[::100,0] + Ringdown[::100,2], 'b-', time[::100], max(Ringdown[:100,0])*np.exp(-time[::100]/2/Q1),'r--')
      plt.xlabel('time'); plt.ylabel('displacement'); plt.title('Ringdown, mode 1')
      #plt.yscale('log')
      plt.subplot(212); 
      plt.plot(time[::100], Ringdown[::100,0] - Ringdown[::100,2], 'g-',time[::100], max(Ringdown[:100,2])*np.exp(-time[::100]/2/Q2),'r--')
      plt.xlabel('time'); plt.ylabel('displacement'); plt.title('Ringdown, mode 2')
      #plt.yscale('log')
      #plt.show()
      
      temp_file_2_q = os.path.join(os.getcwd(),temp_folder) + '/rd_Js_' + str(para) + '.png' 
      plt.savefig(temp_file_2_q)
      plt.close(4)
    ...
    if doRTSA:
      ...
      plt.ylim([1.0, 1.1])
      plt.yscale('linear')
      plt.savefig(os.path.join(os.getcwd(),temp_folder) + '/RTSA_rd_mode1_Js_' + str(para) + '.png', dpi = 400)
      
      RTSA(time, Ringdown[:,2], 200)
      prettify_figure('Frequency', 'time', 'real-time spectrum, ringdown, mode 2')
      plt.ylim([3.0, 3.5])  
      plt.yscale('linear')  
      plt.savefig(os.path.join(os.getcwd(),temp_folder) + '/RTSA_rd_mode2_Js_' + str(para) + '.png', dpi = 400)
    print('elasped time for calculating single ringdown is {} seconds'.format(os.times()[-1] - t_start_ss));

def main():
    
    para_list = np.logspace(-4, 0, 41);   
