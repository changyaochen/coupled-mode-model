...
def process_input(para):
    ...
    if save_file_FFT == 1:
        if not os.path.exists('PythonParallelTemp_bound'):
            os.makedirs('PythonParallelTemp_bound')
        ...
        with open(os.path.join(os.getcwd(),'PythonParallelTemp_bound') + '/' + str(para) + '.txt','w') as FFT_file:
...
def main():
    ...
    with open('combined_bound.txt', 'a') as outfile:
        for para in para_list:
            with open(os.path.join(os.getcwd(),'PythonParallelTemp_bound') + '/' +str(para)+'.txt', 'r') as infile:
