...
cd('/Users/jdong/Box Sync/python_garage/coupled mode model/')

% =========== first steady-state ==================
filename = 'solve_single_case_to_fit_data_ss_trial.';
...
M_ss = RTSA(data_ss(:,[1,2]), timeSlice);
...
% =========== then ringdown ==================
filename = 'solve_single_case_to_fit_data_rd_trial.';
if exist([filename,'mat']) == 0 % the .mat doesn't exist
    filename_h = [filename,'dat'];
    disp('Reading .dat file....');
    tic();
...
M_rd = RTSA(data_rd(:,[1,2]), timeSlice);
...
M_all = RTSA(data_all(:,[1,2]), 2*timeSlice);
prettifyPlot('Frequency', 'Time', 'ringdown');
ylim([61e3, 66e3])
