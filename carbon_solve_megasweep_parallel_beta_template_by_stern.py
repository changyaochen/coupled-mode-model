...
def process_input(para):
    ...
    if save_file_FFT == 1:
        if not os.path.exists('PythonParallelTemp'):
            os.makedirs('PythonParallelTemp')
        ...
        with open(os.path.join(os.getcwd(),'PythonParallelTemp') + '/' + str(para) + '.txt','w') as FFT_file:
...
def main():
    para_list = np.linspace(0, 0.01, 101);   
    ...
    with open('combined_test.txt', 'a') as outfile:
        for para in para_list:
            with open(os.path.join(os.getcwd(),'PythonParallelTemp') + '/' +str(para)+'.txt', 'r') as infile:
                outfile.write(infile.read())
    st.rmtree(os.path.join(os.getcwd(),'PythonParallelTemp'))
